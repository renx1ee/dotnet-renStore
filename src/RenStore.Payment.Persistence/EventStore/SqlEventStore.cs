...
internal sealed class SqlEventStore 
    : RenStore.Payment.Application.Abstractions.IEventStore
{
    private readonly PaymentDbContext _context;

    public SqlEventStore(PaymentDbContext context)
    {
    ...
    private OutboxMessage ToOutboxMessageEntity(
        ...
        DateTimeOffset now)
    {
        if (!IntegrationEventMappings.TypeToName
                .TryGetValue(domainEvent.GetType(), out var type))
        {
            throw new InvalidOperationException(
                $"Event type {domainEvent.GetType()} not registered at in " +
                $"{IntegrationEventMappings.TypeToName}.");
        }
