...
public class DeliveryTariffRepository : IDeliveryTariffRepository
{
    ...
    public async Task<Guid> CreateAsync(DeliveryTariff tariff, CancellationToken cancellationToken)
    {
    ...
    public async Task UpdateAsync(DeliveryTariff tariff, CancellationToken cancellationToken)
    {
    ...
    public async Task<IEnumerable<DeliveryTariff>> FindAllAsync(
        ...
        bool descending = false)
    {
        try
        {
            ...
            return await connection
                .QueryAsync<DeliveryTariff>(sql, new
                {   
                    Count = (int)pageCount,
                    Offset = (int)offset
                });
    ...
    public async Task<DeliveryTariff?> FindByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        if (id == Guid.Empty)
        ...
        try
        {
            ...
            return await connection
                .QueryFirstOrDefaultAsync<DeliveryTariff>(
                    sql, new
                    {
                        Id = id
                    });
    ...
    public async Task<DeliveryTariff?> GetByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        return await this.FindByIdAsync(id, cancellationToken)
               ?? throw new NotFoundException(typeof(DeliveryTariff), id);
    }
}*/