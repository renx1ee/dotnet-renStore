...
public class DeliveryTrackingRepository : IDeliveryTrackingRepository
{
    ...
    public async Task<Guid> CreateAsync(DeliveryTrackingEntity deliveryTracking, CancellationToken cancellationToken)
    {
    ...
    public async Task UpdateAsync(DeliveryTrackingEntity deliveryTracking, CancellationToken cancellationToken)
    {
    ...
    public async Task<IEnumerable<DeliveryTrackingEntity>> FindAllAsync(
        ...
        bool descending = false)
    {
        try
        {
            ...
            return await connection
                .QueryAsync<DeliveryTrackingEntity>(
                    sql, new
                    {
                        Offset = (int)offset,
                        Count = (int)pageCount
    ...
    public async Task<DeliveryTrackingEntity?> FindByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        try
        {
            await using var connection = new NpgsqlConnection(this._connectionString);
            ...
            return await connection
                .QueryFirstOrDefaultAsync<DeliveryTrackingEntity>(
                    sql, new
                    {
                        Id = id
                    });
    ...
    public async Task<DeliveryTrackingEntity?> GetByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        return await this.FindByIdAsync(id, cancellationToken)
               ?? throw new NotFoundException(typeof(DeliveryTrackingEntity), id);
    }

    public async Task<IEnumerable<DeliveryTrackingEntity>> FindByDeliveryOrderId(
        Guid orderId,
        CancellationToken cancellationToken,
        DeliveryTrackingSortBy sortBy = DeliveryTrackingSortBy.Id,
        uint pageCount = 25,
    ...
    public async Task<IEnumerable<DeliveryTrackingEntity>> GetByDeliveryOrderId(
        Guid orderId,
        CancellationToken cancellationToken,
        DeliveryTrackingSortBy sortBy = DeliveryTrackingSortBy.Id,
        uint pageCount = 25,
        ...
        bool descending = false)
    {
        ...
        if(result is null || !result.Any())
               throw new NotFoundException(typeof(DeliveryTrackingEntity), orderId);
